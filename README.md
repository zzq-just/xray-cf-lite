...
2. 卸载            停止 xray + 删除本机 Origin Rules + 恢复 DNS/SSL/安全设置 + 清理本地状态/凭据/订阅快照
3. 查看订阅        显示上次生成的订阅链接
...
| WARP 出站 | 是否让节点流量经 WARP 网卡出站 | 否 |

订阅节点前缀可在安装时设置，也可在主菜单 `4. 修改配置` 中修改。输入 `US` 后，生成的订阅链接会包含 `prefix=US`。

## WARP 出站

安装时可选择 `使用 WARP 出站? (y/N)`。选择 `y` 后，脚本会自动检测已启动的 WARP 网卡并让所有节点入站流量从该网卡出站；无需手动填写网卡名。检测优先匹配 `CloudflareWARP`、`warp`，也兼容以 `cloudflarewarp`、`warp` 或 `wgcf` 开头的名称。未检测到网卡时安装会停止，请先启动 WARP 后重试。
...
- Origin Rules 以每次部署的唯一标识管理；安装、更新和卸载只会修改本机创建的规则，不会回滚或删除其他 VPS、工具或人工创建的规则。
- DNS 记录、SSL 模式和安全设置仍是 Zone 级资源。多台 VPS 共用同一 Zone 时，请为每台 VPS 使用不同子域名，且不要依赖自动恢复这些 Zone 级设置。
